...
//int16_t compute_stop_output(matrix *state, uint16_t precision); // only for budgetrnn
//matrix *fuse_states(matrix *result, matrix *current, matrix *previous, uint16_t precision);  // only for budgetrnn
...
//matrix *pool_logits(matrix *result, matrix logits[NUM_OUTPUTS], matrix states[SEQ_LENGTH], uint16_t n, uint16_t precision); // only for budgetrnn
uint16_t getCurrentLevel(uint16_t seqIndex);
uint8_t isLastSampleInLevel(uint16_t seqIndex, uint16_t fixedSeqIndex);
//int16_t get_state_update_prob(matrix *state, int16_t prevUpdateProb, uint16_t precision); // only for skiprnn
//int16_t binarize_update_prob(int16_t updateProb, uint16_t precision);  // only for skiprnn
