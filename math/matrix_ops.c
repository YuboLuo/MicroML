...
matrix *filter_LEA(matrix* result, matrix *input, matrix *filter, uint16_t precision, uint16_t stride_numRows, uint16_t stride_numCols){
    ...
    uint16_t filter_numRows = filter->numRows;  // we assume filter and input are both square matrix, numRows == numCols
    ...
    uint16_t filter_length = filter_numRows * filter_numCols;  // one filter has 81 bytes
    ...
    if (filter_length & 1) filter_offset = filter_length + 1; // because macParams.length must be a multiple of 2

    dtype *srcA = MULTIPLY_BUFFER;   // srcA saves input
    dtype *srcB = MULTIPLY_BUFFER + filter_offset;  // srcB saves filter
    memset(srcB, 0, sizeof(dtype) * filter_offset);
    memcpy(srcB, filter->data, sizeof(dtype) * filter_offset);
    _iq31 *result_iq31 = srcB + filter_offset;  // msp_mac_q15() returns _iq31 result
    _q15 *result_q15 = result_iq31 + sizeof(result_iq31) * 2;  // we will use msp_iq31_to_q15() after filtering
        ...
        for (j = 0; j <= input_numCols - filter_numCols; j += stride_numCols){
            ...
            for (m = i; m < i + filter_numRows; m ++ ) {
                for (n = j; n < j + filter_numCols; n ++) {  // calculate element-wise matrix product between the filter and corresponding section in the input image
                    ...
                    // copy one number to MULTIPLY_BUFFER from both the input matrix and the filter matrix
                    *(srcA_temp++) = input->data[input_temp];
                }
            }

            // apply hadamard multiplication and sum
